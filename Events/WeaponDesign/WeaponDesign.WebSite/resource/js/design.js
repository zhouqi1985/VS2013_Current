...
var weap = {
    ...
    getUMsg:function(){
        $.ajax({
            ...
            url: 'http://event.zygames.com/qqsm/201512/WeaponDesign/GetAvatar',
            dataType: 'jsonp',
            jsonp: "jsonpcallback",
            async: false, //默认设置下，所有请求均为异步请求。
            success: function (data) {
                var $name=data.Data.AvatarName;
                var serArea=data.Data.GameAreaId;
                if(serArea == 0){serArea = "电信一区";}else if(serArea == 1){serArea = "网通一区";}else if(serArea == 2){serArea = "电信二区";}
                $(".meMsg").html($name+"/"+serArea);
            }
        });
    },
    showConfirm: function (wid) {
        var r = confirm("关闭后无法恢复，确定要这么做么？");
        if (r == true) {
            weap.delWeapon(wid);
            weap.showWeaponList();
        }
    ...
    modify: function (wid) {
        $.ajax({
            ...
            success: function (data) {
                ...
                switch ($WeaType) {
                    case "狙击枪":
                        $(".rInput label:eq(0) input").attr("checked", "checked");
                        break;
                    case "突击枪":
                        $(".rInput label:eq(1) input").attr("checked", "checked");
                        break;
                    case "冲锋枪":
                        $(".rInput label:eq(2) input").attr("checked", "checked");
                        break;
                    case "手枪":
                        $(".rInput label:eq(3) input").attr("checked", "checked");
                        break;
                    default :
                        $(".rInput label:eq(4) input").attr("checked", "checked");
                        $(".typeText").val($WeaType);
                }
                    ...
                    switch ($data[a].StatusType) {
                        case "已审核":
                            $htmls += "<td class='td4'><span class='type1'>已审核</span></td>" +
                                "<td class='td5'>" +
                                "<a target='_blank' href='proShow.html?weaponID=" + $data[a].WeaponID + "' class='btn1'>查看作品</a>" +
                                "<a href='javascript:;' class='btn3' onclick='weap.showConfirm(" + $data[a].WeaponID + ")'>关闭作品</a>" +
                                "</td>" +
                                "</tr>";
                            break;
                        case "已关闭":
                        ...
                        case "未审核":
                            $htmls += "<td class='td4'><span class='type2'>待审核</span><p class='type2'>审核通过，作品上线后作者将无法修改</p></td>" +
                                "<td class='td5'>" +
                                "<a target='_blank' href='proShow.html?weaponID=" + $data[a].WeaponID + "' class='btn1'>查看作品</a>" +
                                "<a target='_blank' href='modify.html?weaponID=" + $data[a].WeaponID + "' class='btn2'>修改作品</a>" +
                                "<a href='javascript:;' class='btn3' onclick='weap.showConfirm(" + $data[a].WeaponID + ")'>关闭作品</a>" +
                                "</td>" +
                                "</tr>";
                            break;
    ...
    addComment: function (weaponID, comment, $url) { //添加评论
        $.ajax({
            ...
            success: function (data) {
                ...
                if ($isok == 0) {
                    alert("添加成功。");
                    weap.showCommentsList(weaponID, 1, 5);
                }
                if ($isok == 1) {
                    weap.islogin($url);
                }
            ...
            success: function (data) {
                ...
                if (rowCount > 0) {
                    for (var j = 0; j < pageCount; j++) {
                        pages += "<a href='javascript:void(0);' onclick='weap.showCommentsList(" + weapId + "," + (j + 1) + ",5)'>" + (j + 1) + "</a>";
                    }
    ...
    islogin: function ($url, $fn) { //判断是否登录
        $.ajax({
            type: "post",
            contentType: 'application/json; charset=UTF-8',
